...
#define DIE_ON_ERROR(x)                 \
    ...
    void orchestrator::destroy_sockets()
    {
        ...
        if (listen_fd_pcap_ != -1)
        {
            close(listen_fd_pcap_);
        }
        if (listen_fd_ctrl_ != -1)
        {
            close(listen_fd_ctrl_);
        }
    }
    ...
    {
        m.set_fragment_id(fid);
        m.set_code(true, type);
    ...
    error_code orchestrator::foreach_fragment_send_ctrl(
        const message::type type, const std::function<void(const uint16_t, message &)> &lambda)
    {

        std::vector<int> fds; // Prepare fds to use with helper
        for (const auto &v : fragments_)
        {
            fds.push_back(v.fd_ctrl);
        }
    ...
    error_code orchestrator::foreach_fragment_recv_generic(
        ...
        const message::type type, const std::function<error_code(const uint16_t, const message &)> &lambda,
        const error_code connection_error)
    {
        ...
        std::unique_ptr<pollfd[]> pfds = (std::make_unique<pollfd[]>(num_fds));
...
#define DIE_DURING_ACCEPT(x)            \
    ...
    error_code orchestrator::run_state_setup_pcap()
    {
        ...
        DIE_DURING_ACCEPT(foreach_fragment_send_ctrl(message::type::
                                                         SETUP_PCAP,
                                                     [this, pcap_netaddr](const uint16_t, message &m)
                                                     {
...
#undef DIE_DURING_ACCEPT
    ...
    orchestrator::run_state_create_topology()
    {
        ...
        return foreach_fragment_recv_ctrl(message::type::TOPOLOGY, [this](const uint16_t, const message &)
                                          { return error_code::NONE; });
    }
    ...
    orchestrator::run_state_start_mixnet_server()
    {
        ...
        // Send the message to every fragment
        DIE_ON_ERROR(foreach_fragment_send_ctrl(
            message::type::START_MIXNET_SERVER, [this](const uint16_t, message &) {}));
                                       ...
                                       const message &m)
        {
            ...
            fragments_[idx].mixnet_server_netaddr.sin_addr.s_addr = baseaddr.sin_addr.s_addr;
    ...
    orchestrator::run_state_start_mixnet_clients()
    {
        ...
        auto send_lambda = [this, topology](const uint16_t idx, message &m)
        {
            ...
            {
                server_netaddrs[nid] = fragments_[topology[idx][nid]].mixnet_server_netaddr;
            }
        };
        // Send the message to every fragment
        DIE_ON_ERROR(foreach_fragment_send_ctrl(
    ...
    orchestrator::run_state_resolve_mixnet_connections()
    {
        ...
        auto lambda = [this, topology](const uint16_t idx, message &m)
        {
            ...
            for (size_t i = 0; i < topology[idx].size(); i++)
            {
                ...
                for (size_t j = 0; j < topology[fragment_id].size(); j++)
                {
                    ...
                    {
                        client_netaddrs[i] = fragments_[fragment_id].mixnet_client_netaddrs[j];
        ...
        // Wait for acknowledgement
        return foreach_fragment_recv_ctrl(
            message::type::RESOLVE_MIXNET_CONNS, [this](const uint16_t, const message &)
            { return error_code::NONE; });
    }
    ...
    orchestrator::run_state_start_testcase()
    {
        ...
        // Wait for acknowledgement
        return foreach_fragment_recv_ctrl(
            message::type::START_TESTCASE, [this](const uint16_t, const message &)
            { return error_code::NONE; });
    }
    ...
    void orchestrator::pcap_thread_loop()
    {
        ...
        std::unique_ptr<pollfd[]> pfds = (std::make_unique<pollfd[]>(num_fragments));
        ...
        while (pcap_thread_run_ && (error_code == error_code::NONE))
        {
            ...
            if (rc < 0)
            {
                error_code = error_code::POLL_ERROR;
            }

            for (size_t fid = 0; fid < num_fragments; fid++)
            {
                const bool is_subscribed = (fragments_[fid].is_pcap_subscribed);

                // Set up for next poll iteration
                pfds[fid].fd = (is_subscribed ? fragments_[fid].fd_pcap : -1);
    ...
    error_code orchestrator::run(testing::testcase &testcase)
    {
        ...
        while (!done)
        {
            ...
            case state_t::INIT:
            {
                ...
                next_state = (error_code == error_code::NONE) ? state_t::SETUP_CTRL_PLANE : state_t::FORCEFUL_SHUTDOWN;
            }
            break;
            ...
            case state_t::SETUP_CTRL_PLANE:
            {
                ...
                next_state = (error_code == error_code::NONE) ? state_t::SETUP_PCAP_PLANE : state_t::FORCEFUL_SHUTDOWN;
            }
            break;
            ...
            case state_t::SETUP_PCAP_PLANE:
            {
                ...
                next_state = (error_code == error_code::NONE) ? state_t::CREATE_MIXNET_TOPOLOGY : state_t::GRACEFUL_SHUTDOWN;
            }
            break;
            ...
            case state_t::CREATE_MIXNET_TOPOLOGY:
            {
                ...
                next_state = (error_code == error_code::NONE) ? state_t::START_ALL_MIXNET_SERVERS : state_t::GRACEFUL_SHUTDOWN;
            }
            break;
            ...
            case state_t::START_ALL_MIXNET_SERVERS:
            {
                ...
                next_state = (error_code == error_code::NONE) ? state_t::START_ALL_MIXNET_CLIENTS : state_t::GRACEFUL_SHUTDOWN;
            }
            break;
            ...
            case state_t::START_ALL_MIXNET_CLIENTS:
            {
                ...
                next_state = (error_code == error_code::NONE) ? state_t::RESOLVE_MIXNET_CONNECTIONS : state_t::GRACEFUL_SHUTDOWN;
            }
            break;
            ...
            case state_t::RESOLVE_MIXNET_CONNECTIONS:
            {
                ...
                next_state = (error_code == error_code::NONE) ? state_t::START_TESTCASE : state_t::GRACEFUL_SHUTDOWN;
            }
            break;
            ...
            case state_t::START_TESTCASE:
            {
                ...
                next_state = (error_code == error_code::NONE) ? state_t::RUN_TESTCASE : state_t::END_TESTCASE;
            }
            break;
            ...
            case state_t::GRACEFUL_SHUTDOWN:
            {
                ...
                next_state = (error_code == error_code::NONE) ? state_t::RESET : state_t::FORCEFUL_SHUTDOWN;
            }
            break;
        ...
        const uint16_t idx, const bool subscribe)
    {
        ...
        auto &current_subscription = (fragments_[idx].is_pcap_subscribed);
                              ...
                              const std::string data_string)
    {
                       ...
                       type, data_string](message &m)
        {
            ...
            payload->type = type;
            payload->src_mixaddr = (testcase_->get_graph().get_node(src_idx).mixaddr());

            payload->dst_mixaddr = (testcase_->get_graph().get_node(dst_idx).mixaddr());
